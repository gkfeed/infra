...
| `item_hash` | `id INTEGER`, `hash TEXT`, nullable `feed_id INTEGER` | Read, insert, and assign hashes to a feed | Parser | PK `id`; FK `feed_id -> feed.id ON DELETE CASCADE`; unique `(feed_id, hash)` |

All columns are non-null except `item_hash.feed_id`. New hashes include a feed
ID. The parser only reads `feed`; test helpers account for its feed writes.
